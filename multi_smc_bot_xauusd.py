"""
XAUUSD-Only SMC Bot with Detailed Logging
- Shows exactly why trades are not placed
- M15 analysis for SMC zones
- M1 execution only when price touches detected zones
- FOK filling mode
- Cooldown handling
- CSV logging
"""
...
# --------------------
def main():
    print("🚀 Starting XAUUSD SMC Bot (Debug Mode)")
    ...
    try:
        while True:
            ...
            if smc_info:
                ...
                reasons = []
                if not inside_any:
                    reasons.append("price not inside zone")
                if not can_trade():
                    reasons.append("cannot trade now")
                if current_time - last_trade_time <= COOLDOWN:
                    reasons.append(f"cooldown active ({int(COOLDOWN - (current_time - last_trade_time))}s left)")

                log(f"ℹ️ {SYMBOL}: Signal={smc_info['signal']}, Price={current_price}, InsideZone={inside_any}")

                if inside_any and can_trade() and current_time - last_trade_time > COOLDOWN:
                    log(f"💡 Placing {smc_info['signal'].upper()} order for {SYMBOL}")
